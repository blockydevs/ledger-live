...
describe("listOperations pagination", () => {
  ...
  beforeAll(
    async () => {
      ...
      startMockServer(mockServer);
      mockServer.use(...buildAleoHandlers({ recipient: owner.address, amount: 0 }));
      ...
      const api = createApi(buildAleoCoinConfig(), ALEO.id);

      wholeListing = (await api.listOperations(owner.address, { minHeight: 0 })).items;

      let cursor: string | undefined;
      do {
        const page = await api.listOperations(owner.address, {
          minHeight: 0,
          limit: PAGE_SIZE,
          ...(cursor && { cursor }),
        });
