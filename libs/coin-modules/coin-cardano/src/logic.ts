...
export function extractPaymentKeyFromAddress(address: string): string {
  try {
    ...
    if (!(cardanoAddress instanceof TyphonAddress.RewardAddress)) {
      log("cardano/logic", "Unsupported address type for payment key", {
        address,
    ...
    log("cardano/logic", "Failed to extract payment key from address", {
      address,
...
export function extractStakeKeyFromAddress(address: string): string | undefined {
  try {
    ...
    log("cardano/logic", "Failed to extract stake key from address", {
      address,
      error: error instanceof Error ? error.message : String(error),
    });
    return undefined;
