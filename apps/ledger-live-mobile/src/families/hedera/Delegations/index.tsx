...
function Delegations({ account, delegatedPosition }: Props) {
  ...
  const delegationStatus = getDelegationStatus(delegationWithMeta.validator);
  ...
  const data = useMemo<DelegationDrawerProps["data"]>(() => {
    return [
      ...
      },
      {
        ...
        Component: (
          <Touchable onPress={() => setIsStatusModalOpen(true)} event="DelegationOpenStatusModal">
            <Flex flexDirection="row" alignItems="center" columnGap={4}>
              <DelegationStatusIcon status={delegationStatus} color={colors.live} />
              <Text
                numberOfLines={1}
                fontWeight="semiBold"
                ellipsizeMode="middle"
                ...
                color="live"
              >
                <Trans i18nKey={`hedera.delegatedPositions.details.status.${delegationStatus}`} />
              </Text>
            </Flex>
          </Touchable>
        ),
  ...
  const delegationActions = useMemo<DelegationDrawerActions>(() => {
    ...
    const mapStakeActionToDisabled = {
      ...
      [HEDERA_TRANSACTION_MODES.ClaimRewards]: delegationWithMeta.pendingReward.isZero(),
